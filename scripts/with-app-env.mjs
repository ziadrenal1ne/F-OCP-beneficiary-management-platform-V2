...
/**
 * Resolve a bare command against the workspace-local node_modules/.bin folder
 * before falling back to PATH. This is required when npm launches a wrapper
 * script on Windows: `spawn("vite")` does not reliably resolve the `.cmd`
 * entry in `node_modules/.bin` without an explicit path.
 */
export function resolveCommand(command) {
  ...
  return existsSync(localExecutable) ? localExecutable : command;
}
