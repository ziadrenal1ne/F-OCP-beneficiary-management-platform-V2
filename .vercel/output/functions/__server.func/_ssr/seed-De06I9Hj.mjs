...
import { b as initials, o as ODDS_CATALOG, v as getSql, y as hashPassword$1 } from "./utils-BYOSEtN8.mjs";
import { randomBytes } from "node:crypto";
//#region node_modules/.nitro/vite/services/ssr/assets/seed-De06I9Hj.js
...
async function seedUsers(sql) {
	for (const acc of [{
		...
		password: "admin123",
		name: "Administrateur FOCP"
	}, {
		email: "cooperative@focp.local",
		password: "cooperative123",
		name: "Représentant Al Amal"
	}]) {
		if ((await sql`select id from "user" where email = ${acc.email} limit 1`)[0]) continue;
		const userId = randomBytes(16).toString("hex");
		const accountId = randomBytes(16).toString("hex");
		const hashed = await hashPassword$1(acc.password);
		await sql`
...
async function runSeed() {
	...
	if ((await sql`select id from seed_meta where id = 1`)[0]) {
		await seedUsers(sql);
		return;
	}
	for (const odd of ODDS_CATALOG) await sql`
      insert into odds (code, name_fr, short_name, color)
      values (${odd.code}, ${odd.nameFr}, ${odd.shortName}, ${odd.color})
      on conflict (code) do nothing
    `;
	const oddRows = await sql`select id, code from odds order by code`;
	const now = /* @__PURE__ */ new Date();
	const coopIds = [];
	for (let i = 0; i < NAMES.length; i++) {
		const rng = mulberry32(1e3 + i * 17);
		const loc = CITIES[i % CITIES.length];
		const jitterLat = (rng() - .5) * .12;
		const jitterLng = (rng() - .5) * .12;
		const sector = SECTORS[i % SECTORS.length];
		const statusRoll = rng();
		const status = statusRoll > .92 ? "suspended" : statusRoll > .82 ? "pending" : "active";
		...
		const phone = `+212 5${Math.floor(20 + rng() * 70)} ${String(Math.floor(rng() * 9e6 + 1e6)).slice(0, 7)}`;
		const website = rng() > .35 ? `https://www.${slug || "coop"}.ma` : "";
		const legal = rng() > .2 ? "Coopérative" : "Union de coopératives";
		const coopId = (await sql`
      insert into cooperatives (
        ...
        ${name}, ${`${DESCRIPTIONS[i % DESCRIPTIONS.length]} Basée à ${loc.city}, elle intervient principalement en ${sector.toLowerCase()}.`},
    ...
    `)[0].id;
		...
		const shuffled = [...oddRows].sort(() => rng() - .5).slice(0, oddCount);
		for (const o of shuffled) await sql`insert into cooperative_odds (cooperative_id, odd_id) values (${coopId}, ${o.id}) on conflict do nothing`;
		...
		for (let m = 11; m >= 0; m--) {
			...
			children = Math.max(2, Math.round(children * (1 + (rng() * .03 - .01))));
			disabled = Math.max(1, Math.round(disabled * (1 + rng() * .02)));
		...
		await sql.query(`insert into beneficiary_stats (
        cooperative_id, year, month, women, men, youth, adults, children, disabled, indirect
      ) values ${statValues.join(",")} on conflict (cooperative_id, year, month) do nothing`, statParams);
		...
		for (let r = 0; r < reportCount; r++) {
			...
			if (r === 0 && rng() > .4) rStatus = "submitted";
			else if (statusRollR > .88) rStatus = "rejected";
			else if (statusRollR > .78) rStatus = "draft";
        ...
        insert into reports (
          ...
          ${`Rapport ${d.toLocaleDateString("fr-FR", {
				month: "long",
				year: "numeric"
			})}`},
          ...
          ${rng() > .5 ? "Accès limité à l'eau d'irrigation et volatilité des prix." : "Logistique de transport et besoin en équipements."},
      ...
      `;
		...
		for (let c = 0; c < convCount; c++) {
			...
			const daysLeft = (end.getTime() - now.getTime()) / 864e5;
			const cStatus = daysLeft < 0 ? "expired" : daysLeft < 60 ? "expiring" : "active";
			const amount = (80 + Math.floor(rng() * 420)) * 1e3;
			await sql`
      ...
      `;
		...
		const docs = [
			{
				name: "Statuts de la coopérative.pdf",
				category: "Statuts",
				mime: "application/pdf",
				status: "approved"
			},
			{
				name: "Rapport d'activité 2025.pdf",
				category: "Rapport d'activité",
				mime: "application/pdf",
				status: rng() > .3 ? "approved" : "pending"
			},
			{
				name: "Liste des membres.csv",
				category: "Données CSV",
				mime: "text/csv",
				status: "approved"
			}
		];
      ...
      insert into esg_indicators (
        ...
        ${Math.floor(200 + rng() * 4e3)}, ${Math.floor(100 + rng() * 8e3)},
	...
	await seedUsers(sql);
	const admin = await sql`select id from "user" where email = ${"admin@focp.local"} limit 1`;
	const coopUser = await sql`select id from "user" where email = ${"cooperative@focp.local"} limit 1`;
	const firstCoop = coopIds[0] ?? null;
	if (admin[0]) await sql`
      insert into profiles (user_id, role, cooperative_id, display_name)
      values (${admin[0].id}, ${"admin"}, null, ${"Administrateur FOCP"})
      on conflict (user_id) do nothing
    `;
	if (coopUser[0]) await sql`
      insert into profiles (user_id, role, cooperative_id, display_name)
  ...
  `;
	for (const r of pendingReports) await sql`
  ...
  `;
	for (const c of expiring) await sql`
      insert into notifications (user_id, role_target, cooperative_id, type, title, body, href)
      values (
...
function ensureSeeded() {
	globalRef.__focpSeedPromise__ ??= runSeed().catch((err) => {
		globalRef.__focpSeedPromise__ = void 0;
		throw err;
	});
	return globalRef.__focpSeedPromise__;
}
//#endregion
export { ensureSeeded as n, createServerRpc as t };
